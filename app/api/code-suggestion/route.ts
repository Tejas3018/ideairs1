...
async function generateSuggestion(prompt: string): Promise<string> {
  try {
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-4o", // Note: GPT-4.1 doesn't exist, using GPT-4o which is the latest
      messages: [
        {
          role: "system",
          content: "You are an expert code assistant. Provide concise, accurate code suggestions based on the context. Only return the code suggestion without explanations unless specifically asked."
        },
        {
          role: "user",
          content: prompt
    ...
    let suggestion = response.choices[0]?.message?.content || "// AI suggestion unavailable"
