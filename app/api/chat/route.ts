  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a prompt enhancement assistant. Enhance user prompts to be more specific and effective for coding assistance."
        },
        {
          role: "user",
    ...
    return enhancedPrompt?.trim() || request.prompt
  } catch (error) {
